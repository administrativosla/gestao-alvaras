...
- situacaoCli: para documentos CLI, retorne "parcial" SOMENTE se o documento contiver EXPLICITAMENTE uma marca d'água, tarja ou texto tachado/sobreposto no fundo com as expressões: "PENDENTE DE FINALIZAÇÃO", "documento parcial", "não produz os efeitos legais", "finalizar as licenças dos órgãos integrados". ATENÇÃO CRÍTICA: a simples listagem de órgãos participantes (Prefeitura, Vigilância Sanitária, Bombeiros, etc.) NÃO é sinal de pendência — todos os CLIs listam os órgãos participantes, mesmo os completos. Retorne "parcial" APENAS quando a tarja/marca d'água estiver visualmente presente no documento. Caso contrário, retorne "completo". Para documentos que não são CLI, retorne null.
- cliOrgaosPendentes: SOMENTE para CLI com situacaoCli="parcial" (tarja visível no documento). Identifique os órgãos que ainda NÃO emitiram manifestação definitiva — ou seja, aparecem sem número de documento final, com status "Protocolo", "Indeterminado" ou com anotação explícita de pendência. NÃO inclua órgãos que já emitiram parecer ou declaração. Para CLIs completos ou não-CLI, retorne null.
